...
maximize efficiency:
sum{b in BIDDERS, ep in POSITIVE_HYPEREDGES} positive_applies[b,ep] * positive_valuation[b,ep] -
sum{b in BIDDERS, en in NEGATIVE_HYPEREDGES} negative_applies[b,en] * negative_valuation[b,en];

s.t. assign_once{i in ITEMS}: sum{b in BIDDERS} assigned[b,i] <= 1;

s.t. positive_constraint{b in BIDDERS, e in POSITIVE_HYPEREDGES}:  
...
s.t. negative_constraint{b in BIDDERS, e in NEGATIVE_HYPEREDGES}:  
negative_applies[b,e] + sum{i in ITEMS} occurs_in_negative[i,e] -
sum{i in ITEMS} occurs_in_negative[i,e] * assigned[b,i] >= 1;
